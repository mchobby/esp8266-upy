# --- Follower node  ----------------------------
# The follower mask waits the messages sent by the master node.
# 
# This script uses filter+mask to intercept several messages.
# The messages of interest are messages for "traffic_light_Axxx"
# 
#
...
# Prepare the match (avoids multiple creation). Extended is set to True because we are on 29bits
# As we want to match 0x100FA... for 0x100FAA01 & 0x100FAA01 THEN only the last 17 first bit must
# be the same than the address.
_match1 = Match( address=traffic_light_AA01, mask=0b1111_11111111_11110000_00000000, extended=True )


print('Listening on %s node with mask %s ...' % (hex(_match1.address),bin(_match1.mask)) )
while True:
