...
class MCP2515:
    ...
    def __init__(self, spi_bus: SPI, cs_pin: Pin, *, baudrate: int = 250000, loopback: bool = False,
                 silent: bool = False, silent_pin: Pin = None, debug: bool = False):
        ...
        :param Pin silent_pin: some transceiver do also expose a silent pin. When silent is True \
                then the mentionned Pin is switched.
        ...
        self.spi = spi_bus
        #self.spi.init() Should already be initialised
        
        self.cs = cs_pin
        self.cs.on() # Should already be on

        self._buffer = bytearray(20)
        self._id_buffer = bytearray(4)
        self._unread_message_queue = []
        self._timer = TimeoutTimer()
        self._tx_buffers = []
